...
const getDashboardData = async (req, res) => {
  try {
    ...
    // Fetch recent 10 tasks
    const recentTasks = await Task.find()
      .sort({ createdAt: -1 }) // ✅ use correct field
      .limit(10)
      .select("title status priority dueDate createdAt");

    // ✅ return corrected keys
    res.status(200).json({
      statistics: {
        totalTasks,
        pendingTasks,
      ...
      charts: {
        taskDistribution,
        taskPriorityLevels, // ✅ fixed spelling
      },
      recentTasks,
    });
  } catch (error) {
...
const getUserDashboardData = async (req, res) => {
  try {
    ...
    // ✅ Fetch recent tasks (last 5 assigned to user)
    const recentTasks = await Task.find({ assignedTo: userId })
      .sort({ createdAt: -1 }) // newest first
      .limit(5)
      .select("title status priority dueDate createdAt"); // only send needed fields
    ...
    res.json({
      ...
      recentTasks, // 👈 now frontend will have data
    });
  } catch (error) {
